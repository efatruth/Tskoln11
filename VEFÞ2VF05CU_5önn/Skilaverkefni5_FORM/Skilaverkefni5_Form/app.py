...
info = {'namskeid':['Gagnasafn','Python','javascript']}

@route("/tuna")
def index():
        return template("./views/index.tpl ",namskeid=info)

@route("/send", method="POST")
def form_process():
        form_list = request.forms.getall('namskeid')
        print(form_list)
...
def form_process():
    ...
    if request.POST.get("submit","").strip():
        ...
        if valid:
            return template('signup_success.tpl', name = new_user[0],address = new_user[1],email = new_user[2], phone= new_user[3], username = new_user[4],password = new_user[5])
        else:
            return redirect("/")
